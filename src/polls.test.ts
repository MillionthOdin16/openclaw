...
describe("polls", () => {
  describe("resolvePollMaxSelections", () => {
    it("returns Math.max(2, optionCount) when allowMultiselect is true", () => {
  ...
  it("rejects when there are less than 2 options after cleaning", () => {
    expect(() =>
  ...
  it("handles missing options gracefully", () => {
    expect(() =>
      // @ts-expect-error Intentionally missing options for coverage of fallback
      normalizePollInput({ question: "Q" }),
    ).toThrow("Poll requires at least 2 options");
  });
