...
describe("cli credentials", () => {
  ...
  it("reads Codex credentials from keychain when available", async () => {
    ...
    // the hash depends on the temp path, so we don't mock the specific hash
    // we just want to ensure it has the correct prefix

    execFileSyncMock.mockImplementation((file: unknown, args: unknown) => {
      expect(String(file)).toBe("security");
      const argv = Array.isArray(args) ? args.map(String) : [];
      expect(argv).toContain("Codex Auth");
      const accountArg = argv[argv.indexOf("-a") + 1];
      expect(accountArg).toMatch(/^cli\|/);
      return JSON.stringify({
        tokens: {
