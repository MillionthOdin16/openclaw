...
function randomChoice(values: string[], fallback: string) {
  return values[crypto.randomInt(0, values.length)] ?? fallback;
}
...
export function createSessionSlug(isTaken?: (id: string) => boolean): string {
  ...
  const fallback = `${createSlugBase(3)}-${crypto.randomBytes(2).toString("hex").slice(0, 3)}`;
  return isIdTaken(fallback) ? `${fallback}-${crypto.randomBytes(4).toString("hex")}` : fallback;
}
