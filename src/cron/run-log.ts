...
export async function readCronRunLogEntriesPageAll(
  ...
  // ⚡ Bolt Optimization: Filter data concurrently inside the promise mapping
  // and push directly to a flat array using a loop. This prevents huge memory spikes
  // from intermediate array allocations and avoids RangeErrors that occur when
  // flattening or spreading massive datasets.
  const filtered: CronRunLogEntry[] = [];
  await Promise.all(
    jsonlFiles.map(async (filePath) => {
      const raw = await fs.readFile(filePath, "utf-8").catch(() => "");
      const parsed = parseAllRunLogEntries(raw);
      const filteredChunk = filterRunLogEntries(parsed, {
        statuses,
        deliveryStatuses,
        query,
      ...
      for (const entry of filteredChunk) {
        filtered.push(entry);
      }
    }),
  );
