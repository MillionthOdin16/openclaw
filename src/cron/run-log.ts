...
async function parseAllRunLogEntriesStream(
  filePath: string,
  opts?: { jobId?: string },
): Promise<CronRunLogEntry[]> {
...
export async function readCronRunLogEntriesPage(
  ...
  const all = await parseAllRunLogEntriesStream(path.resolve(filePath), { jobId: opts?.jobId });
...
export async function readCronRunLogEntriesPageAll(
  ...
  const chunks = await Promise.all(
    jsonlFiles.map(async (filePath) => {
      return parseAllRunLogEntriesStream(filePath);
    }),
  );
