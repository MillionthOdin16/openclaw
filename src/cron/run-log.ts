...
async function streamAndFilterRunLogEntries(
  filePath: string,
  opts: {
    jobId?: string;
...
export async function readCronRunLogEntriesPage(
  ...
  const filtered = await streamAndFilterRunLogEntries(path.resolve(filePath), {
    jobId: opts?.jobId,
    statuses,
    deliveryStatuses,
    query,
...
export async function readCronRunLogEntriesPageAll(
  ...
  const chunks = await Promise.all(
    jsonlFiles.map((filePath) =>
      streamAndFilterRunLogEntries(filePath, {
        statuses,
        deliveryStatuses,
        query,
